...
export const IPTVPage = () => {
  ...
  const allChannels = useMemo(() => {
    let queued = [];
    try {
      queued = JSON.parse(localStorage.getItem('pending_channel_requests') || '[]');
    } catch {
      queued = [];
    }
  ...
  const { healthMap, isChecking, progress, checkAllChannels, getStats, getHealth } =
  ...
  const applySavedDiscovery = useCallback(channel => {
    ...
    return {
      ...
      discoveryMeta: {
        ...
        verified:
          typeof bestCandidate.health?.isAlive === 'boolean' ? bestCandidate.health.isAlive : null,
        cacheKey,
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      setLoading(false);
      logger.warn('IPTV player did not become ready within 10s — hiding spinner');
    }, 10000);
  ...
  useEffect(() => {
    ...
    if (meta.restoredFromCache) {
      ...
      setDiscoveryStatus(`Using cached backup stream${meta.source ? ` (${meta.source})` : ''}`);
    }
  }, [selectedChannel]);
  ...
  const persistDiscoveryCache = useCallback((immediate = false) => {
    ...
    const run = () => {
      ...
      try {
            ...
            .map(item => ({
              ...
              health:
                item.health && typeof item.health === 'object'
                  ? {
                      isAlive:
                        typeof item.health.isAlive === 'boolean' ? item.health.isAlive : null,
        ...
        logger.warn('[IPTV] Failed to persist discovery cache', error);
      }
    };
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.warn('[IPTV] Failed to restore discovery cache', error);
    }
  }, []);
  ...
  const recordDiscoverySuccess = useCallback(
    channel => {
      ...
      const enrichedEntry = {
        ...
        health: (() => {
          ...
          if (sourceMeta.health && typeof sourceMeta.health === 'object') {
            if (typeof sourceMeta.health.isAlive === 'boolean')
              output.isAlive = sourceMeta.health.isAlive;
      ...
      const filteredQueue = queue
        .filter(
          (entry, index) =>
            entry && entry.url && index !== existingIndex && entry.url !== successUrl
        )
      ...
      setDiscoveryStatus(`Stream verified${label ? ` (${label})` : ''}`);
    },
    [persistDiscoveryCache]
  );
  ...
  const categories = useMemo(
    () => ['all', ...new Set(allChannels.map(c => c.category).filter(Boolean))],
    [allChannels]
  );
  ...
  const toggleFavorite = useCallback((ch, e) => {
    e.stopPropagation();
    setFavorites(prev =>
      prev.includes(ch.name) ? prev.filter(n => n !== ch.name) : [...prev, ch.name]
    );
  }, []);
  ...
  const attemptDiscoveryFallback = useCallback(
    (reason, options = {}) => {
      ...
      if (currentSnapshot?.name) {
        const prefix =
          reason === 'manual-discovery' ? 'Searching backups for' : 'Recovering stream for';
      ...
      const run = (async () => {
          ...
          results.forEach(candidate => {
            ...
            candidate.streams.forEach(stream => {
              ...
              queue.push({
                url: rawUrl,
                type:
                  stream.streamType && stream.streamType !== 'unknown'
                    ? stream.streamType
                    : undefined,
                requiresProxy: Boolean(stream.requiresProxy),
        ...
        if (isStale) {
          ...
          try {
            ...
            logger.warn('[IPTV] Discovery fallback fetch failed', error);
            setLoading(false);
            setIsDiscoveryChecking(false);
        ...
        if (!nextCandidate) {
          logger.warn('[IPTV] No discovery fallback streams available for', currentChannel.name);
          setLoading(false);
          setIsDiscoveryChecking(false);
        ...
        logger.info('[IPTV] Switching to discovery fallback stream', nextCandidate.url);
        setLoading(true);
        setSelectedChannel(channelState => {
          ...
          const nextChannel = {
            ...
            discoveryMeta: {
              ...
              reason,
              verified:
                typeof nextCandidate.health?.isAlive === 'boolean'
                  ? nextCandidate.health.isAlive
                  : null,
              cacheKey,
      ...
      discoveryInFlightRef.current = run
        .catch(error => {
          logger.warn('[IPTV] Discovery fallback failed', error);
          setLoading(false);
          setIsDiscoveryChecking(false);
  ...
  const onPlayerError = useCallback(
    fallbackPayload => {
      const payload =
        typeof fallbackPayload === 'string' ? { url: fallbackPayload } : fallbackPayload || {};
      const fallbackUrl = payload?.url;

      if (!fallbackUrl) {
        logger.warn(
          'Player error with no static fallback URL available. Triggering discovery.',
          payload.reason || ''
        );
        setIsDiscoveryChecking(true);
      ...
      logger.warn('Player error, attempting fallback:', fallbackUrl);
      setLoading(true);
      ...
      setSelectedChannel(currentChannel => {
        ...
        if (
          currentChannel.url === fallbackUrl &&
          (!payload.type || currentChannel.type === payload.type)
        ) {
          setLoading(false);
        ...
        const originalSource =
          payload.originalUrl || currentChannel.originalUrl || currentChannel.url;
        if (originalSource) {
    ...
    <div className="px-4 sm:px-8 md:px-16 pt-28 pb-20 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        {getStats.checked > 0 && (
          <div className="bg-(--bg-secondary) rounded-xl p-4 mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="text-center md:text-right">
              <div className="text-2xl font-bold text-(--brand-color)">{getStats.percentage}%</div>
        ...
        <div className="grid gap-6 md:grid-cols-3 mb-6">
          {/* Player (wide) */}
          <div className="md:col-span-2 bg-(--bg-secondary) rounded-2xl overflow-hidden shadow-2xl">
            <div className="bg-linear-to-r from-(--brand-color) to-red-700 p-4">
      ...
      <ChannelRequestModal
        ...
        onSuccess={channel => {
          try {
            ...
            logger.warn('Failed to add user channel locally', e);
          }
        }}
      />
    </div>
