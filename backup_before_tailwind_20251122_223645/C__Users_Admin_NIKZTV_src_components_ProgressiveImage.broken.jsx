...
export const OptimizedPoster = ({ item, onClick, isWatched, isLarge = false, showTitle = true }) => {
  ...
  const placeholderUrl = item?.backdrop_path || item?.poster_path
    ? `https://image.tmdb.org/t/p/w92${item.backdrop_path || item.poster_path}`
    : '/placeholder.png';

  return (
    <div className="relative cursor-pointer group transform transition-all duration-300 hover:scale-105 hover:z-10" onClick={() => onClick && onClick(item)}>
      <ProgressiveImage
        src={imageUrl}
      ...
      {isWatched && isWatched(item?.id) && (
        <div className="absolute top-2 right-2 bg-green-600 text-white px-2 py-1 rounded-full text-xs font-bold"> Watched</div>
      )}
      ...
      {showTitle && (
        <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/80 to-transparent p-3 rounded-b-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <h3 className="text-white font-semibold text-sm line-clamp-2">{item?.title || item?.name}</h3>
