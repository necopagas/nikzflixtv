// Manganelo API Proxy
// This handles scraping Manganelo for manga reading
...
async function handleSearch(req, res, query) {
  ...
  try {
    const response = await fetch(
      `https://manganelo.com/search/story/${encodeURIComponent(query)}`,
      {
...
async function handlePopular(req, res) {
  try {
    const response = await fetch(
      'https://manganelo.com/manga_list?type=topview&category=all&state=all&page=1',
      {
        headers: { 'User-Agent': 'Mozilla/5.0' },
      }
    );
...
async function handleSeriesInfo(req, res, mangaId) {
  ...
  try {
    const response = await fetch(`https://manganelo.com/manga/${mangaId}`, {
      headers: { 'User-Agent': 'Mozilla/5.0' },
    });
...
async function handleChapterPages(req, res, chapterId) {
  ...
  try {
    const response = await fetch(`https://manganelo.com/chapter/${chapterId}`, {
      headers: { 'User-Agent': 'Mozilla/5.0' },
    });
