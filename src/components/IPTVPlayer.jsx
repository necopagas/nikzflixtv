...
import {
  normalizeHeaders,
  buildDashRequestModifier,
  applyDashProtectionData,
  buildShakaDrmConfiguration,
} from '../utils/iptvDrm';
import {
  PiArrowsOutSimple,
  PiArrowsInSimple,
...
const resolveExplicitType = (channel = {}) => {
  ...
  if (['dash', 'mpd', 'mpeg-dash', 'dashjs', 'dash/shaka', 'dash_shaka'].includes(value))
    return 'dash';
...
export const IPTVPlayer = ({ channel, isLoading: parentLoading, onCanPlay, onError }) => {
  ...
  const initializePlayer = useCallback(() => {
    ...
    const attemptFallback = (reason, urlOverride = null, delayMs = 0) => {
      ...
      const addCandidate = (candidateUrl, detectionUrl) => {
        ...
        const typeHint = detectStreamType(
          detectionUrl || extractOriginalUrl(candidateUrl, candidateUrl)
        );
    ...
    const startPlaybackMonitor = () => {
      ...
      playbackGuardRef.current = setTimeout(() => {
        ...
        if (!playbackConfirmed && !fallbackTriggered) {
          attemptFallback(
            `[IPTV] Playback stalled for ${channel?.name || channel.url}. Switching to fallback if available.`
          );
        }
      }, guardDelay);
    };
    ...
    try {
      if (type === 'hls') {
        if (Hls.isSupported()) {
          ...
          const mergedConfig = channel?.hls?.config
            ? { ...hlsConfig, ...channel.hls.config }
            : hlsConfig;
          ...
          hls.on(Hls.Events.MANIFEST_PARSED, () => {
            ...
            setQualities([
              { label: 'Auto', value: -1 },
              ...hls.levels.map((l, i) => ({ label: `${l.height}p`, value: i })),
            ]);
          ...
          hls.on(Hls.Events.ERROR, (event, data) => {
            ...
            if (data.fatal && data.type === Hls.ErrorTypes.NETWORK_ERROR) {
              attemptFallback(
                `[IPTV] HLS network error for ${channel?.name || channel.url}.`,
                channel.fallback
              );
            } else if (data.fatal) {
              attemptFallback(
                `[IPTV] HLS fatal error for ${channel?.name || channel.url}.`,
                channel.fallback,
                200
              );
            }
          ...
          video.addEventListener('error', () => {
            ...
            attemptFallback(
              `[IPTV] Native HLS error for ${channel?.name || channel.url}.`,
              channel.fallback
            );
          });
        } else {
        ...
        const startDashWithDashJS = () => {
          ...
          try {
            ...
            if (typeof channel?.dash?.requestModifier === 'function') {
              ...
              player.extend(
                'RequestModifier',
                buildDashRequestModifier(dashHeaders, dashWithCredentials),
                true
              );
            }
            ...
            player.on(MediaPlayer.events.QUALITY_CHANGE_RENDERED, e => {
              if (e.mediaType === 'video') {
                ...
                if (currentBitrate) {
                  setCurrentQuality(
                    player.getSettings().streaming.abr.autoSwitchBitrate.video
                      ? 'Auto'
            ...
            player.on(MediaPlayer.events.ERROR, e => {
              ...
              attemptFallback(
                `[IPTV] dash.js error for ${channel?.name || channel.url}.`,
                channel.fallback,
                400
              );
            });
            ...
            attemptFallback(
              `[IPTV] dash.js initialization failed for ${channel?.name || channel.url}.`,
              channel.fallback
            );
            return false;
          }
          ...
          try {
            ...
            const handleShakaFatal = error => {
              ...
              if (!dashFallbackTried) {
                ...
                if (!dashStarted) {
                  attemptFallback(
                    `[IPTV] Shaka fatal error, dash.js unavailable for ${channel?.name || channel.url}.`,
                    channel.fallback,
                    500
                  );
                }
              ...
              attemptFallback(
                `[IPTV] Shaka fatal error for ${channel?.name || channel.url}.`,
                channel.fallback,
                500
              );
            };
            ...
            player.addEventListener('adaptation', () => {
              ...
              if (activeTrack) {
                setCurrentQuality(
                  abrEnabled
                    ? 'Auto'
                    : `${activeTrack.height || Math.round((activeTrack.bandwidth || 0) / 1000)}p`
                );
            ...
            attemptFallback(
              `[IPTV] Shaka initialization failed for ${channel?.name || channel.url}.`,
              channel.fallback
            );
            return false;
          }
        ...
        video.addEventListener('error', () => {
          ...
          attemptFallback(
            `[IPTV] Progressive playback error for ${channel?.name || channel.url}.`,
            channel.fallback
          );
        });
      } else {
  ...
  const switchQuality = value => {
    ...
    if (hlsRef.current) {
      ...
      setCurrentQuality(
        levelIndex === -1 ? 'Auto' : selected.label || `${levelInfo?.height || ''}p`
      );
      ...
      if (selected.value === -1) {
        ...
        const targetTrack = tracks.find(
          track => String(track.id ?? track.height) === String(selected.trackId ?? selected.value)
        );
        if (targetTrack) {
          shakaRef.current.selectVariantTrack(targetTrack, true, 0);
          setCurrentQuality(
            selected.label ||
              `${targetTrack.height || Math.round((targetTrack.bandwidth || 0) / 1000)}p`
          );
    ...
    >
      {(parentLoading || isSwitching) && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/80 z-20 pointer-events-none">
          <div className="w-12 h-12 border-4 border-t-(--brand-color) border-gray-600 rounded-full animate-spin"></div>
        </div>
      )}
      ...
      <div
        className={`absolute inset-0 bg-linear-to-t from-black/70 via-transparent to-transparent transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      >
        {/* Top Controls */}
        <div className="absolute top-0 left-0 right-0 p-3 flex justify-end text-white z-10">
          <div className="flex gap-2">
            <button
              ...
              className="p-2 rounded-full bg-black/50 hover:bg-white/20 transition-colors focus:outline-none focus:ring-2 focus:ring-(--brand-color)"
            ...
            >
              {isFullscreen ? (
                <PiArrowsInSimple className="w-5 h-5" />
              ) : (
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 text-white z-10">
          <div className="flex justify-between items-center mb-2">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 ml-2">
                ...
                <input
                  ...
                  className="w-24 focus:outline-none focus:ring-2 focus:ring-(--brand-color)"
                />
              </div>

              <h3
                className="font-bold text-lg truncate max-w-[calc(100%-150px)]"
                aria-live="polite"
              >
            ...
            <div className="flex items-center gap-2 shrink-0">
              {latency !== null && !isNaN(latency) && (
                <span
                  className={`text-xs px-2 py-1 rounded ${latency < 5 ? 'bg-green-600' : latency < 15 ? 'bg-orange-600' : 'bg-red-600'}`}
                >
              ...
              {qualities.length > 1 && (
                <button
                  ...
                  aria-haspopup="menu"
                  aria-label={`Quality ${currentQuality}`}
                  className="px-3 py-1 bg-white/20 hover:bg-white/30 rounded text-sm focus:outline-none focus:ring-2 focus:ring-(--brand-color)"
                >
                  {currentQuality}{' '}
                  <span className="ml-1 text-xs">{showQualityMenu ? '▾' : '▴'}</span>
                </button>
              )}
                  ...
                  <div className="absolute right-0 bottom-10 bg-black/90 rounded p-2">
                    {[0.5, 0.75, 1, 1.25, 1.5, 2].map(rate => (
                      <button
                        ...
                        className={`block w-full text-left px-3 py-1 text-sm hover:bg-white/20 ${playbackRate === rate ? 'text-(--brand-color) font-bold' : ''}`}
                      >
                        {rate}x
                      </button>
                    ))}
              ...
              <button
                ...
                className={`p-2 rounded-full bg-black/50 hover:bg-white/20 transition-colors focus:outline-none focus:ring-2 focus:ring-(--brand-color) ${captionsEnabled ? 'text-(--brand-color)' : ''}`}
          ...
          {showQualityMenu && qualities.length > 1 && (
            <div
              className="absolute bottom-14 right-3 bg-black/90 rounded-lg p-2 min-w-[120px] max-h-48 overflow-y-auto z-50"
              role="menu"
              ...
              {qualities.map(q => (
                <button
                  ...
                  className={`block w-full text-left px-3 py-2 rounded hover:bg-white/20 text-sm transition-colors ${
                    (currentQuality === q.label && q.label !== 'Auto') ||
                    (currentQuality === 'Auto' && q.value === -1)
                      ? 'text-(--brand-color) font-bold'
                      : ''
                  }`}
                >
                  {q.label}
      ...
      {isMobile && (
        <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 z-30 bg-black/60 rounded-full px-3 py-2 flex items-center gap-3 backdrop-blur-md">
          <button
            onClick={() => seek(-10)}
            aria-label="Rewind 10 seconds"
            className="p-2 rounded-full bg-black/40 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-(--brand-color)"
          >
            <FaBackward />
          </button>
          <button
            ...
            aria-pressed={isPlaying}
            aria-label={isPlaying ? 'Pause' : 'Play'}
            className="p-3 rounded-full bg-(--brand-color) text-black focus:outline-none focus:ring-2 focus:ring-white"
          >
            {isPlaying ? <FaPause /> : <FaPlay />}
          </button>
          <button
      ...
      {showStats && (
        <div className="absolute top-4 left-4 bg-black/80 text-green-400 text-xs p-3 rounded font-mono space-y-1 z-20">
          <p>
            Buffer: {bufferHealth !== null && !isNaN(bufferHealth) ? `${bufferHealth}s` : 'N/A'}
          </p>
