...
export const SettingsModal = ({
  ...
  const [autoSkipIntro, setAutoSkipIntro] = useState(
    localStorage.getItem('autoSkipIntro') !== 'false'
  );
  const [autoSkipOutro, setAutoSkipOutro] = useState(
    localStorage.getItem('autoSkipOutro') === 'true'
  );
  const [skipIntroTime, setSkipIntroTime] = useState(
    parseInt(localStorage.getItem('skipIntroTime') || '90')
  );
  const [subtitleSize, setSubtitleSize] = useState(
    parseInt(localStorage.getItem('subtitleSize') || '100')
  );
  const [parentalControl, setParentalControl] = useState(
    localStorage.getItem('parentalControl') === 'true'
  );
  const [maturityRating, setMaturityRating] = useState(
    localStorage.getItem('maturityRating') || 'all'
  );

  const handleClearData = (clearFunction, type) => {
    if (
      window.confirm(`Are you sure you want to clear your ${type}? This action cannot be undone.`)
    ) {
      clearFunction();
    }
  };
  ...
  return (
    ...
    >
      <div
        className="bg-linear-to-br from-[var(--bg-secondary)] to-[var(--bg-tertiary)] w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl relative overflow-hidden border border-(--border-color)"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="bg-linear-to-r from-red-900/20 to-red-600/20 p-6 border-b border-(--border-color)">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <FaCog className="text-3xl text-red-500 animate-spin-slow" />
              <h2 className="text-3xl font-bold bg-linear-to-r from-red-500 to-red-300 bg-clip-text text-transparent">
                Settings
              </h2>
            </div>
            <button
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg font-semibold transition-all whitespace-nowrap ${
                    activeTab === tab.id
                      ...
                      : 'bg-(--bg-tertiary) text-gray-400 hover:bg-(--bg-tertiary-hover) hover:text-white'
                  }`}
                >
                  <Icon />
                  {tab.label}
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
            ...
            <div className="space-y-6 animate-fade-in">
              <SettingCard title="Language" icon={FaLanguage}>
                <select
                  ...
                  className="w-full bg-(--bg-tertiary) border border-(--border-color) rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-red-500 focus:outline-none"
                >
            ...
            <div className="space-y-6 animate-fade-in">
              <SettingCard title="Theme" icon={FaPalette}>
                <div className="flex gap-3">
                  <button
                    onClick={toggleTheme}
                    className="flex-1 px-6 py-3 rounded-lg font-semibold transition-all border-2 bg-linear-to-r from-gray-900 to-black hover:scale-105"
                  >
              ...
              <SettingCard title="🎉 Seasonal Themes" icon={FaSnowflake}>
                <div className="space-y-3">
                  ...
                  {isChristmasMode && (
                    <div className="mt-3 space-y-3 p-3 bg-linear-to-br from-red-900/20 to-green-900/20 rounded-lg border border-yellow-500/30">
              ...
              <SettingCard title="Subtitle Size">
                <div className="space-y-2">
                  <label className="text-sm text-gray-400">Font Size: {subtitleSize}%</label>
                  <input
                    type="range"
                    min="50"
                    max="200"
                    ...
                    className="w-full h-2 bg-(--bg-tertiary) rounded-lg appearance-none cursor-pointer accent-red-600"
                  />
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Preview:{' '}
                    <span style={{ fontSize: `${subtitleSize}%` }}>Sample subtitle text</span>
                  </p>
                </div>
              </SettingCard>
            </div>
                    ...
                    <button
                      ...
                      className={`px-4 py-3 rounded-lg font-semibold transition-all ${
                        videoQuality === quality
                          ? 'bg-red-600 text-white shadow-lg shadow-red-600/50'
                          : 'bg-(--bg-tertiary) text-gray-400 hover:bg-(--bg-tertiary-hover)'
                      }`}
                    >
                      {quality.toUpperCase()}
                    </button>
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <SettingCard title="Data Management">
                <div className="space-y-3">
                  <button
                    onClick={() =>
                      handleClearData(onClearContinueWatching, 'Continue Watching list')
                    }
                    className="w-full text-left bg-(--bg-tertiary) hover:bg-(--bg-tertiary-hover) p-3 rounded-lg transition-all font-medium"
                  >
          ...
          {activeTab === 'about' && (
            <div className="space-y-6 animate-fade-in">
              <div className="bg-linear-to-br from-red-900/20 to-purple-900/20 rounded-2xl p-8 text-center border border-red-500/30">
                <div className="text-6xl mb-4">🎬</div>
                <h3 className="text-3xl font-bold mb-2 bg-linear-to-r from-red-500 to-purple-500 bg-clip-text text-transparent">
                  NikzFlix TV
                </h3>
              ...
              <div className="bg-black/30 rounded-lg p-6">
                ...
                <ul className="space-y-2 text-gray-300 text-sm">
                  <li>
                    • Developed by <span className="text-red-500 font-semibold">NikzFlix Team</span>
                  </li>
...
const SettingCard = ({ title, icon: Icon, children }) => (
  <div className="bg-(--bg-tertiary)/50 backdrop-blur-sm rounded-xl p-6 border border-(--border-color)">
