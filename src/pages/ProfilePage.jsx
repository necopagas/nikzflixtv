...
export const ProfilePage = () => {
  ...
  const handleUsernameUpdate = async e => {
    ...
    try {
      ...
      setUsernameSuccess(
        'Username updated successfully! You may need to refresh the page to see changes across the site.'
      );
    } catch (err) {
  ...
  return (
    <div className="flex justify-center min-h-screen pt-28 pb-20 px-4">
      <div className="w-full max-w-md space-y-8">
        {/* --- USERNAME FORM --- */}
        <div className="p-8 space-y-6 bg-(--bg-secondary) rounded-lg shadow-lg">
          <h1 className="text-2xl font-bold text-center text-(--text-primary)">My Profile</h1>
          <div className="text-center border-b border-(--border-color) pb-4">
            <p className="text-lg text-(--text-secondary)">Email Address</p>
            <p className="text-xl font-semibold text-(--text-primary) break-words">
              {currentUser.email}
            </p>
          </div>
          <form onSubmit={handleUsernameUpdate} className="space-y-6">
            <h2 className="text-xl font-bold text-center text-(--text-primary)">Display Name</h2>
            <div>
            ...
            {usernameSuccess && (
              <p className="text-green-500 text-sm text-center">{usernameSuccess}</p>
            )}
            <button
              type="submit"
        ...
        <div className="p-8 space-y-6 bg-(--bg-secondary) rounded-lg shadow-lg">
          <form onSubmit={handlePasswordUpdate} className="space-y-6">
            <h2 className="text-xl font-bold text-center text-(--text-primary)">Change Password</h2>
            <div>
