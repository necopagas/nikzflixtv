...
export const DramaDetailPage = () => {
    ...
    useEffect(() => {
        ...
        fetchData(API_ENDPOINTS.details('tv', dramaId)) 
            .then(data => {
                if (data && data.id) { // Check kung valid ba ang response
    ...
    if (!drama) {
        return (
            <div className="text-center py-16 pt-28">
                <p className="text-xl text-[var(--text-secondary)]">Drama details not found.</p>
                <button onClick={() => navigate('/drama')} className="mt-4 px-4 py-2 bg-red-600 rounded">Back to Dramas</button>
            </div>
        );
    }
        ...
        <div className="px-4 sm:px-8 md:px-16 pt-28 pb-20">
            <div className="flex flex-col md:flex-row gap-8">
                <div className="md:w-1/3 flex-shrink-0">
                    {drama.image ? (
                         <img src={drama.image} alt={drama.title} className="w-full rounded-lg shadow-lg aspect-[2/3] object-cover" />
                    ): (
                        <div className="w-full rounded-lg shadow-lg aspect-[2/3] bg-[var(--bg-tertiary)] flex items-center justify-center">
                ...
                <div className="md:w-2/3">
                    ...
                    {drama.otherNames && drama.otherNames.length > 0 && (
                        <p className="text-lg text-[var(--text-secondary)] mb-4">
                            Also known as: {drama.otherNames.slice(0, 3).join(', ')} {/* Limit alternative titles */}
                        </p>
                    )}
                    <div className="flex flex-wrap gap-2 mb-4">
                     ...
                     {drama.numberOfSeasons && (
                         <p className="text-sm text-[var(--text-secondary)] mb-4">{drama.numberOfSeasons} Season{drama.numberOfSeasons > 1 ? 's' : ''}</p>
                     )}
                    <p className="text-[var(--text-primary)] leading-relaxed">{drama.description || "No description available."}</p>
