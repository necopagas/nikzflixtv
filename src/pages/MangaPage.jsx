      ...
      if (data) {
        ...
        const normalizedData = data
          .map(item => ({
            id: item.id,
            title:
              typeof item.title === 'object' ? item.title.romaji || item.title.english : item.title,
  ...
  return (
    <div className="px-4 sm:px-8 md:px-16 pt-28 pb-20">
      <div className="mb-8 text-center">
        ...
        <p className="text-lg text-(--text-secondary)">Powered by Consumet API</p>
      </div>
      ...
      <div className="flex justify-center mb-12">
        <input
          ...
          className="w-full max-w-lg p-3 bg-(--bg-tertiary) rounded-md focus:outline-none focus:ring-2 focus:ring-(--brand-color) transition-all"
        />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
          {mangaList.map(manga => (
            <div
              key={manga.id}
              className="poster-wrapper group shrink-0 relative cursor-pointer"
            ...
            >
              <ProgressiveImage
                ...
                imgProps={{
                  className:
                    'poster w-full h-full object-cover rounded-md transition-transform duration-300 group-hover:scale-110',
                  loading: 'lazy',
                  onError: e => {
