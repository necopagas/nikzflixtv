...
const EXTERNAL_IPTV_URL = 'https://raw.githubusercontent.com/aCoDenz/w5s/refs/heads/main/livetv/TVChannels.json';
...
const getCategoryFromName = (name) => {
    ...
    // News
    if (lowerName.includes('news') || lowerName.includes('aljazeera') || 
        lowerName.includes('bloomberg') || lowerName.includes('bbc news') || lowerName.includes('sky news')) {
        return 'News';
    }
    
    // Kids
    if (lowerName.includes('kids') || lowerName.includes('disney') || 
        lowerName.includes('nickelodeon') || lowerName.includes('nick jr') || 
        lowerName.includes('cartoon') || lowerName.includes('baby') || 
        lowerName.includes('zoomoo') || lowerName.includes('kidoodle')) {
        return 'Kids';
    }
    ...
    // Sports
    if (lowerName.includes('nba') || lowerName.includes('nhl') || 
        lowerName.includes('combat') || lowerName.includes('redbull') || lowerName.includes('sport')) {
        return 'Sports';
    }
    
    // Music
    if (lowerName.includes('mtv') || lowerName.includes('music') || 
        lowerName.includes('hallypop') || lowerName.includes('frontrow')) {
        return 'Music';
    }
    
    // Documentary
    if (lowerName.includes('discovery') || lowerName.includes('nat geo') || 
        lowerName.includes('national geographic') || lowerName.includes('animal planet') || 
        lowerName.includes('travel') || lowerName.includes('wild earth') || lowerName.includes('lotus')) {
        return 'Documentary';
    }
    
    // Premium
    if (lowerName.includes('hbo') || lowerName.includes('showtime') || 
        lowerName.includes('starz') || lowerName.includes('cinemax')) {
        return 'Premium';
    }
    
    // Movies
    if (lowerName.includes('movie') || lowerName.includes('amc') || 
        lowerName.includes('scream') || lowerName.includes('thriller') || 
        lowerName.includes('action') || lowerName.includes('plex') || lowerName.includes('samsung')) {
        return 'Movies';
    }
...
export const checkIPTVUpdates = (localChannels, externalChannels) => {
    ...
    const externalMap = new Map(
        externalChannels.map(ch => [ch.name.toUpperCase(), ch.url])
    );
